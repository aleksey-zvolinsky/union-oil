...
begin
  ...
  // Получаем номер эмитента (первые четыре цифры карточки)
  if not TempForm.getEmitentNumber then
  begin
    MessageDlg(TranslateText('  Не могу получить код эмитента! ')+#10#13 +
              TranslateText('  Свяжитесь с разработчиками для получения пакета, ')+#10#13 +
              TranslateText(' содержащего код эмитента.'), mtError, [mbOK], 0);
